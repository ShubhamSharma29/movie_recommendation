...
getwd()
setwd("F:/VIT BHOPAL DOCUMENTS/Projects/Big Data-Movie Recommendation/DataSet/")
...
# we need to convert the genres present in the movie_data dataframe
# into a more usable format by the users.
...
# we will create a ‘search matrix’ that will allow us to perform an easy search of 
...
#table to display the most unique ratings
Table_of_Ratings <- table(rating_values) # creating a count of movie ratings
Table_of_Ratings



#Most viwed movies visualization
...
#bar plor for total number of views of the film
ggplot(table_views[1:6, ], aes(x = title, y = views)) +
  geom_bar(stat="identity", fill = 'steelblue') +
  geom_text(aes(label=views), vjust=0.0, size=3.5) +
...
#the algorithm will now identify the k most similar
#items and store their number. I will use the cosine method to do so.
...
#we will carry out the sum of rows and columns with the similarity
#of the objects above 0. We will visualize the sum of columns through a distribution:
...
# qplot(item_count, fill=I("steelblue"), col=I("red"))+
#   ggtitle("Distribution of the number of item for IBCF")